...
public final class ReciprocalArraySum {
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        double value = 0.0;
        List<ReciprocalArraySumTask> subTasks = new ArrayList<>(numTasks);

        for (int i = 0; i < numTasks; i++) {
            int start = getChunkStartInclusive(i, numTasks, input.length);
            int end = getChunkEndExclusive(i, numTasks, input.length);
            ReciprocalArraySumTask currTask = new ReciprocalArraySumTask(start, end, input);
            currTask.fork();
            subTasks.add(currTask);
        }

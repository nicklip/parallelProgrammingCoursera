...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
        ...
        double retVal = Stream.of(studentArray).parallel().filter(s -> s.checkIsCurrent())
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
        ...
        String retVal = Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent())
                        .collect(Collectors.groupingBy((Student student) -> student.getFirstName(), Collectors.counting()))
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
        ...
        //throw new UnsupportedOperationException();
        System.setProperty("java.util.concurrent.ForkJoinPool.common.parallelism", "3");
        int retVal = (int) Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent())
                    .filter(a -> a.getAge() > 20).filter(s -> s.getGrade() < 65).count();
        return retVal;
    }
}
